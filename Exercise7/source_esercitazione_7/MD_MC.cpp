...
//Read input informations
  ...
  ReadInput >> rcut;
  cout << "Cutoff of the interatomic potential = " << rcut << endl << endl; //c
...
//Read initial configuration
  ...
  if(restart)
  {
    ...
    cout<<"read from previous config"<<endl;
...
// distance i-j in pbc
      ...
      if(dr < rcut)
      {
        ...
        pij = 48*(1.0/pow(dr,12) - 0.5/(pow(dr,6)));
        pressure+=pij;
        v += vij;
      }
      //radial distribution function
      
	    drbin = (box*0.5)/(double)(nbins);
	    for(int k=0; k<nbins; k++){
		    if ((drbin*k <= dr) and (dr < (drbin*(k+1)))){
			    g_blk[k] +=2.;
		    }				
  ...
  double tc_pot, tc_pres;
  //tail corrections 
  tc_pot=8*npart*M_PI*rho*(1/(9*pow(rcut, 9))-1/(3*pow(rcut, 3)));
  tc_pres=96*npart*M_PI*rho*(1/(9*pow(rcut, 9))-1/(3*pow(rcut, 3)));
  
  walker[iv] = 4.0 * v + tc_pot; // Potential energy
  walker[ik] = kin; // Kinetic energy
  walker[it] = (2.0 / 3.0) * kin/(double)npart; // Temperature
  walker[ie] = 4.0 * v + kin + tc_pot;  // Total energy;
  walker[ip] = (pressure + tc_pres)/(3*vol);
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Pressure, acceptance_ratio, dis_func, dis_func_f;
    
    cout << "Block number " << iblk << endl;
    cout << "Acceptance rate " << accepted/attempted << endl << endl;
    
    Epot.open("output_epot.txt", ios::app);
    Ekin.open("output_ekin.txt", ios::app);
    Temp.open("output_temp.txt", ios::app);
    Etot.open("output_etot.txt", ios::app);
    Pressure.open("output_pressure.txt", ios::app);
    //dis_func.open("output_dis_func.txt", ios::app);
    dis_func_f.open("output_dis_func_final.txt", ios::app);
    ...
    stima_pressure = rho*stima_temp + blk_av[ip]/(blk_norm); //Pressure
    glob_av[ip] += stima_pressure;
    glob_av2[ip] += stima_pressure*stima_pressure;
...
//Pressure
    Pressure  << iblk <<  " " << stima_pressure << " " << glob_av[ip]/(double)iblk <<" "<< err_press << endl;

    cout << "----------------------------" << endl << endl;

  double rt;
    
    for(int i=0; i<nbins; i++){ //distribution function
      rt=i*drbin;
      rt= (4.*M_PI/3.)*(pow((rt+drbin),3.) - pow(rt,3.)); 
      normal = rho*npart*rt;
      stima_g = g_blk[i]/(blk_norm*normal); 
    
      g_glob[0][i] += stima_g;
      g_glob[1][i] += pow(stima_g,2.);
    ...
    if (iblk == nblk){
      ...
      acceptance_ratio << delta << ' ' << (double)tot_acc/(double)(tot_att) <<endl;
      for(int i=0; i<nbins; i++){
 	      dis_func_f << i*drbin <<' '<< g_glob[0][i]/(double)iblk <<' '<<Error(g_glob[0][i],g_glob[1][i],nblk) << endl;
 	    }
