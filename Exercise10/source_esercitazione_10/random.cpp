...
int Random :: Check_int(std::vector<int> &v){
    int counter = 0;
    if(v[0]==1){}
    else{counter++;}
    ...
    if(counter!=0){
        cout<<"Problem, check this member of the population (Check int)"<<endl;
        return 0;
    }
    else{
        return 1;
...
void Random :: Check_s(std::vector<int> &v){
    int counter = 0;
    if(v[0]==1){}
    else{counter++;
    cout<<"1"<<endl;}
    ...
    if(counter!=0){
        cout<<"Problem, check this member of the population shift"<<endl;
    }
}
...
std::vector<int> Random::permutation (std::vector<int> &v){
        ...
        int e;
        for(int i =0; i<5; i++ ){
            c = (unsigned int)Rannyu(1, 49);
            d = (unsigned int)Rannyu(1, 49);
            e = v[c];
            //std::cout<<c<<" "<<d<<endl;
            v[c] = v[d];
            v[d] = e;   
...
std::vector<int> Random:: shift (std::vector<int> &v, int m){
    std::vector<int> v2(51);
    int h;
    
    
    do{ h = (int)Rannyu(1, 24);}while(m+h>49);
    for(int i=0; i<50; i++){
        v2[i]=v[i];
    }
    int e;
...
std::vector<int> Random::permutation_m (std::vector<int> &v){
...
//order, better using the void version!
...
std::vector<int> Random:: crossover1(std::vector<int> &gen1, std::vector<int> &gen2){
    ...
    int l = int(Rannyu(1, 50));
    for(int y=0; y<50-l; y++){
    ...
    Check_c(crossover1);
    //control=Check_int(crossover1);
    return crossover1;
}  
//void order
void Random::order2(std::vector<vector<int>> &pop, int n, vector<double> x, vector<double> y){
